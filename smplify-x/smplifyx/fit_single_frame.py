...
def fit_single_frame(img,
    ...
    with fitting.FittingMonitor(
        ...
        for or_idx, orient in enumerate(tqdm(orientations, desc='Orientation')):
            ...
            if use_vposer:
                # body_pose = vposer.decode(pose_embedding)['pose_body'].view(1, -1)
                body_pose = vposer.decode(pose_embedding)['pose_body'].reshape(1, -1)
                # result['body_pose'] = pose_embedding.detach().cpu().numpy()
                result['body_pose'] = body_pose.detach().cpu().numpy()  # ⭕ 올바르게 저장됨
    ...
    if save_meshes or visualize:
        # body_pose = vposer.decode(pose_embedding)['pose_body'].view(1, -1) if use_vposer else None
        body_pose = vposer.decode(pose_embedding)['pose_body'].reshape(1, -1) if use_vposer else None
