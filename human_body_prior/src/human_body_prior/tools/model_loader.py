...
def load_config(config_path):
    ...
    config_dict['torch_device'] = 'cuda' if torch.cuda.is_available() else 'cpu'

    return DotMap(config_dict)

# def load_vposer(vposer_path, vp_model='snapshot', vp_checkpoint='epoch_20.pth'):
#     vp_conf = load_config(os.path.join(vposer_path, 'config.yaml'))
#     vp_weights = os.path.join(vposer_path, vp_model, vp_checkpoint)

#     vposer = VPoser(vp_conf)
#     # vposer.load_state_dict(torch.load(vp_weights, map_location=vposer.device))
#     device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
#     vposer.load_state_dict(torch.load(vp_weights, map_location=device))
#     clean_state_dict = {}
#     for k, v in state_dict.items():
#         if k.startswith("vp_model."):
#             clean_key = k[len("vp_model."):]
#             clean_state_dict[clean_key] = v
#         else:
#             clean_state_dict[k] = v

#     # vposer.load_state_dict(clean_state_dict)
#     vposer.load_state_dict(clean_state_dict, strict=False)
...
def load_vposer(vposer_path, vp_model='snapshot', vp_checkpoint='epoch_20.pth'):
    ...
    # ✅ state_dict 먼저 저장
    state_dict = torch.load(vp_weights, map_location=device)
