...
#         self.mu = nn.Linear(num_feat_in, latentD).to(dtype=self.dtype, device=self.device),
#         self.logvar = nn.Linear(num_feat_in, latentD).to(dtype=self.dtype, device=self.device)
class NormalDistDecoder(nn.Module):
    def __init__(self, num_feat_in, latentD):
        super(NormalDistDecoder, self).__init__()
...
class VPoser(nn.Module):
    def __init__(self, model_ps):
        ...
        if hasattr(model_ps, 'model_params'):
            num_neurons = model_ps.model_params.num_neurons
            self.latentD = model_ps.model_params.latentD
        else:
            # fallback: 평평한 config 구조
            num_neurons = model_ps.num_neurons
            self.latentD = model_ps.latentD
        ...
        #     nn.Linear(n_features, num_neurons).to(dtype=self.dtype, device=self.device),
        #     nn.LeakyReLU(),
        ...
        #     nn.Linear(self.latentD, num_neurons).to(dtype=self.dtype, device=self.device),
        #     nn.LeakyReLU(),
        #     nn.Dropout(0.1),
        #     nn.Linear(num_neurons, num_neurons).to(dtype=self.dtype, device=self.device),
        #     nn.LeakyReLU(),
        #     nn.Linear(num_neurons, self.num_joints * 6).to(dtype=self.dtype, device=self.device),
        #     ContinousRotReprDecoder(),
        # )
        l1 = nn.Linear(n_features, num_neurons)
        l2 = nn.Linear(num_neurons, num_neurons)
        l3 = nn.Linear(num_neurons, num_neurons)
        ...
        self.encoder_net = self.encoder_net

        d1 = nn.Linear(self.latentD, num_neurons)
        d2 = nn.Linear(num_neurons, num_neurons)
        d3 = nn.Linear(num_neurons, self.num_joints * 6)
        ...
        self.decoder_net = self.decoder_net
